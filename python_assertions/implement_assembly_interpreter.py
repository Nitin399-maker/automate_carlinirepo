...
# Test squares
print("TEST1:", evaluate(\"\"\"{}\"\"\"", memory, registers)[:10])
...
# Test primes
print("TEST2:", evaluate(\"\"\"{}\"\"\"", memory, registers)[:10])
""".format(code1, primes_code)
...
def evaluate(program: str, memory: List[int], registers: Dict[str, int]) -> List[int]:
    ...
    while pc < len(instructions):
        ...
        if op == 'SET':
            ...
            registers['flag'] = 1 if registers[instr[1]] > registers[instr[2]] else 0
            pc += 1
        elif op == 'EQ':
            registers['flag'] = 1 if registers[instr[1]] == registers[instr[2]] else 0
            pc += 1
            ...
            addr = registers[instr[2]]
            while len(memory) <= addr:
                memory.append(0)
            memory[addr] = registers[instr[1]]
            pc += 1
        elif op == 'JMP':
            ...
            if instr[1] not in labels:
                raise RuntimeError(f'Unknown label: {instr[1]}')
            pc = labels[instr[1]] if registers.get('flag', 0) == 1 else pc + 1
        elif op == 'JF':
            if instr[1] not in labels:
                raise RuntimeError(f'Unknown label: {instr[1]}')
            pc = labels[instr[1]] if registers.get('flag', 0) == 0 else pc + 1
        else:
